...
function askQuestion(question) {  
    var answer = prompt(nextQuestion.text);
    return answer;
}
...
function runQuiz(questions) {
    ...
    while (hasNextQuestion(shuffledQuestions) && isTimerEnabled()) {
        var nextQuestion = getNextQuestion(shuffledQuestions);
        var answer = askQuestion(nextQuestion);        
        if (isAnswerCorrect(nextQuestion, answer)) {
            answeredCorrect++;
        }
    }
